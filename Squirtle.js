//Initialisation

var textBank = document.getElementById("wordlist.txt").files[0];
var ovarall = FileReader.readAsText(textBank);
//var overallDict = readFileSync("./wordlist.txt").toString('utf-8');
var answerBank = ['water', 'juice', 'urine', 'honey', 'blood', 'fluid', 'sauce', 'booze', 'cider', 'broth', 'bevvy', 'lager', 'lassi', 'latte', 'pimms', 'punch', 'stout', 'tonic', 'vodka', 'hooch', 'juicy', 'drink', 'turps', 'pepsi', 'fanta'];

print(overallDict);
